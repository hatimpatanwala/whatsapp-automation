...
export const NODE_TYPE_DEFINITIONS: NodeTypeDefinition[] = [
  ...
  // Commerce nodes
  {
    ...
    configFields: [
      ...
      { key: 'header', label: 'List Header', type: 'text', defaultValue: 'Browse our products:', variables: true, help: 'Text shown above the product list.' },
      { key: 'buttonText', label: 'List Button Text', type: 'text', defaultValue: 'View Products', help: 'Label on the button that opens the list (max 20 chars).' },
      { key: 'sectionTitle', label: 'Section Title', type: 'text', defaultValue: 'Products' },
      { key: 'currencySymbol', label: 'Currency Symbol', type: 'text', defaultValue: '₹', help: 'Shown before each price.' },
      { key: 'emptyMessage', label: 'If catalog empty', type: 'text', defaultValue: 'Our catalog is currently empty. Please check back later!', variables: true },
    ],
  },
  {
    type: 'add_to_cart',
  ...
  },
  {
    ...
    maxOutputs: 2,
    configFields: [
      { key: 'header', label: 'Cart Header', type: 'text', defaultValue: '🛒 Your Cart:', variables: true },
      { key: 'totalLabel', label: 'Total Label', type: 'text', defaultValue: 'Total' },
      { key: 'currencySymbol', label: 'Currency Symbol', type: 'text', defaultValue: '₹' },
      { key: 'lineFormat', label: 'Line Format', type: 'text', defaultValue: '• {name} × {qty} — {currency}{subtotal}', help: 'Placeholders: {name} {qty} {price} {subtotal} {currency}' },
      { key: 'showCheckout', label: 'Show Checkout Button', type: 'boolean', defaultValue: true },
      { key: 'checkoutLabel', label: 'Checkout Button Label', type: 'text', defaultValue: 'Checkout' },
      { key: 'showClear', label: 'Show Clear Cart Button', type: 'boolean', defaultValue: true },
      { key: 'clearLabel', label: 'Clear Button Label', type: 'text', defaultValue: 'Clear Cart' },
      { key: 'continueLabel', label: 'Continue Shopping Label', type: 'text', defaultValue: 'Continue Shopping' },
      { key: 'emptyMessage', label: 'If cart empty', type: 'text', defaultValue: 'Your cart is empty. Browse our catalog to add items!', variables: true },
    ],
  },
  {
    type: 'checkout',
  ...
  },
  {
    ...
    maxOutputs: 2,
    configFields: [
      { key: 'showImage', label: 'Show Product Image', type: 'boolean', defaultValue: true },
      { key: 'showPrice', label: 'Show Price', type: 'boolean', defaultValue: true },
      { key: 'showDescription', label: 'Show Description', type: 'boolean', defaultValue: true },
      { key: 'descriptionLimit', label: 'Description Max Length', type: 'number', defaultValue: 350 },
      { key: 'currencySymbol', label: 'Currency Symbol', type: 'text', defaultValue: '₹' },
      { key: 'inCartText', label: '"In Cart" Text', type: 'text', defaultValue: '🛒 In your cart: *{qty}*', help: 'Use {qty} for the quantity in cart.' },
      { key: 'addToCartLabel', label: 'Add to Cart Button', type: 'text', defaultValue: '🛒 Add to Cart', help: 'Max 20 chars.' },
      { key: 'viewCartLabel', label: 'View Cart Button', type: 'text', defaultValue: '🛒 View Cart', help: 'Max 20 chars.' },
      { key: 'keepShoppingLabel', label: 'Keep Shopping Button', type: 'text', defaultValue: '🛍️ Keep Shopping', help: 'Max 20 chars.' },
      { key: 'incLabel', label: 'Add One Button', type: 'text', defaultValue: '➕ Add one', help: 'Shown when item is already in cart.' },
      { key: 'decLabel', label: 'Remove One Button', type: 'text', defaultValue: '➖ Remove one', help: 'Shown when item is already in cart.' },
      { key: 'unavailableMessage', label: 'If Product Unavailable', type: 'text', defaultValue: 'Sorry, that product is no longer available. Send *menu* to browse.', variables: true },
    ],
  },
  {
    type: 'my_orders',
  ...
  },
  {
    ...
    maxOutputs: 1,
    configFields: [
      { key: 'steps', label: 'Status Stages', type: 'text', defaultValue: 'placed,confirmed,processing,ready_for_delivery,delivered', help: 'Comma-separated order stages shown in the progress bar.' },
      { key: 'currencySymbol', label: 'Currency Symbol', type: 'text', defaultValue: '₹' },
      { key: 'askMessage', label: 'If No Order Number', type: 'text', defaultValue: 'Please send your *order number* (e.g. ORD-ABC123), then try again.', variables: true },
      { key: 'notFoundMessage', label: 'If Order Not Found', type: 'text', defaultValue: '❓ No order found for *{order}*. Please double-check the number.', help: 'Use {order} for the number they typed.' },
      { key: 'cancelledMessage', label: 'If Cancelled', type: 'text', defaultValue: '❌ Order *{order}* was cancelled.', help: 'Use {order} for the order number.' },
      { key: 'statusTemplate', label: 'Status Message', type: 'textarea', defaultValue: '🚚 *Order {order}*\nStatus: *{status}*\nTotal: {currency}{total}\n\n{progress}', help: 'Placeholders: {order} {status} {currency} {total} {progress}' },
    ],
  },
