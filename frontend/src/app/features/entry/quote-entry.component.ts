...
/**
 * Quotation — keyboard grid using the same billing intelligence as the sales screen
 * (stock + party-wise last rate while quoting). Saves a draft quote via the existing
 * quotes module, which the web portal can then send over WhatsApp.
 */
@Component({
  selector: 'wa-quote-entry',
  standalone: true,
  ...
  template: `
    <div class="p-3 md:p-5 max-w-6xl select-none">
      ...
      <div class="flex items-center gap-2 mb-3">
        ...
        @if (customer(); as c) {
          <span class="text-sm text-slate-500">{{ c.totalOrders }} orders · outstanding ₹{{ fmt(c.outstanding) }}</span>
        }
      </div>
      ...
      <table class="w-full text-sm border border-slate-300" style="border-collapse: collapse">
        ...
        <tbody>
          @for (row of rows; track $index; let r = $index) {
            ...
            @if (row.productId && row.lastToCustomer) {
              <tr><td></td>
      ...
      <div class="flex gap-4 mt-3 items-start">
        ...
        <div class="text-sm w-64 space-y-1">
          ...
          <button (click)="save()" [disabled]="saving() || !canSave()"
            ...
            {{ saving() ? 'Saving…' : 'Save Quotation (Ctrl+A)' }}
          </button>
        </div>
      </div>
...
export class QuoteEntryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const d = this.drafts.load<any>('quote');
    if (!d) return;
  ...
  onProductQuery(r: number, q: string): void {
    ...
    row.productId = undefined; row.stock = undefined; row.lastToCustomer = null;
    clearTimeout(this.debounce);
  ...
  private loadItemContext(r: number, prefillRate = false): void {
    ...
    this.entry.itemContext(row.productId, this.customer()?.id).subscribe((ctx) => {
      ...
      // Rate priority: price level → party's last rate → standard (already prefilled).
  ...
  /** Combined single-percentage equivalent of the D1×D2 cascade (quote items carry one discount field). */
  private combinedDisc(row: Row): number {
    const eff = (1 - (Number(row.d1) || 0) / 100) * (1 - (Number(row.d2) || 0) / 100);
    return Math.round((1 - eff) * 10000) / 100;
  }
    ...
    this.entry
      .createQuote({
        ...
        items: this.liveRows().flatMap((r) => {
          const lines: Array<{ productId?: string; description: string; quantity: number; unitPrice: number; discount?: number }> = [{
            productId: r.productId,
            description: r.name,
            quantity: Number(r.qty),
            unitPrice: Number(r.rate) || 0,
            discount: this.combinedDisc(r), // percentage — matches QuoteService semantics
          }];
          // Miracle scheme: free quantity as a ₹0 ride-along line.
          if (Number(r.free) > 0) {
            lines.push({ productId: r.productId, description: `${r.name} (FREE)`, quantity: Number(r.free), unitPrice: 0 });
          }
          return lines;
        }),
      } as any)
      .subscribe({
        ...
        error: (err) => {
          this.saving.set(false);
          this.error.set(err?.error?.message || 'Failed to save quotation');
        },
      });
  }
}
